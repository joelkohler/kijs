...
kijs.gui.field.MultiCombo = class kijs_gui_field_MultiCombo extends kijs.gui.field.Field {
    ...
    constructor(config = {}) {
        ...
        this._inputField = new kijs.gui.Dom({
            disableEscBubbeling: true,
            nodeTagName: 'input'
        });
        ...
        this._listViewEl = new kijs.gui.ListView({
            autoScroll: false,
            cls: 'kijs-field-multicombo',
            autoLoad: false,
            focusable: false,
            selectType: 'simple'
        });

        this._mainListViewEl = new kijs.gui.ListView({
            autoScroll: false,
            autoLoad: false,
            focusable: false
        });
...
//        this._eventForwardsAdd('escPress', this._inputDom);
        ...
        // Listeners
        this._inputField.on('blur', this._onInputBlur, this);
        this._inputField.on('change', this._onInputChange, this);
        this._inputField.on('input', this._onInputInput, this);
        this._inputField.on('keyUp', this._onInputKeyUp, this);
        this._inputField.on('keyDown', this._onInputKeyDown, this);
    ...
    get captionField() {
        return this._listViewEl.captionField;
    }

    set captionField(val) {
        this._listViewEl.captionField = val;
    }

    get valueField() {
        return this._listViewEl.valueField;
    }

    set valueField(val) {
        this._listViewEl.valueField = val;
    }
    ...
    get facadeFnArgs() {
        return this._listViewEl.facadeFnArgs;
    }

    set facadeFnArgs(val) {
        this._listViewEl.facadeFnArgs = val;
    }

    get facadeFnLoad() {
        return this._listViewEl.facadeFnLoad;
    }

    set facadeFnLoad(val) {
        this._listViewEl.facadeFnLoad = val;
    }
    ...
    // overwrite
    get isEmpty() {
        return kijs.isEmpty(this.value);
    }
    ...
    get rpc() {
        return this._listViewEl.rpc;
    }

    set rpc(val) {
        this._listViewEl.rpc = val;
    }
    ...
    set value(val) {
        ...
        let elements = [];
        this._mainListViewEl.removeAll();
        kijs.Array.each(val, function (key) {
            elements.push(
                {
                    xtype: 'kijs.gui.Container',
                    cls: 'kijs-value-element',
                    name: key,
                    elements: [
                        {
                            xtype: 'kijs.gui.Element',
                            html: this._getCaptionFromValue(key)
                        }, {
                            xtype: 'kijs.gui.Icon',
                            iconMap: 'kijs.iconMap.Fa.xmark',
                            on: {
                                click: this._onValueElementCloseClick,
                                context: this
                            }
                        }
                    ]
        ...
        this._mainListViewEl.add(elements);
...
//                this._listViewEl.data = [];
    ...
    render(superCall) {
        ...
        // ListView rendern
        this._mainListViewEl.renderTo(this._inputDom.node);

        // Input Dom rendern
        this._inputField.renderTo(this._inputDom.node);
     ...
     */
    _setProposal(key) {
        let inputVal = this._inputField.nodeAttributeGet('value'),
            matchVal = '';
        ...
        if (inputVal && key !== 'Backspace' && key !== 'Delete') {
            ...
            if (matchVal) {
                this._inputField.nodeAttributeSet('value', kijs.toString(matchVal));

                // Differenz selektieren
                if (matchVal.length !== inputVal.length) {
                    this._inputField.node.setSelectionRange(inputVal.length, matchVal.length);
                }
            }
    ...
    _validationRules(value) {
        ...
        if (this._forceSelection && !this._remoteSort && !kijs.isEmpty(value)) {
            let match = false;
            kijs.Array.each(this.value, function (row) {
                kijs.Array.each(this._listViewEl.data, function (data) {
                    if (data[this.valueField] === row) {
                        match = true;
                        return false;
                    }
                }, this);
    ...
    _onInputKeyDown(e) {
        ...
        console.log(this._listViewEl.getSelected());
        // Scroll
        ...
        if (e.nodeEvent.key === 'Enter') {
            ...
            if (dataViewElement && dataViewElement instanceof kijs.gui.DataViewElement) {
                ...
                this.value = this._value.push(newVal);
    ...
    _onInputKeyUp(e) {
        ...
        this._keyUpDefer = kijs.defer(function () {
            if (this._remoteSort) {
                this.load(null, false, this._inputField.nodeAttributeGet('value'));
            } else {
    ...
    _onValueElementCloseClick(e) {
        let value = e.element.parent.name;

        let values = this.value;
        this.value = kijs.Array.remove(values, value);
    }
    ...
    destruct(superCall) {
        ...
        this._value = null;
        this._keyUpDefer = null;
        this._remoteSort = null;
        this._forceSelection = null;
        this._firstLoaded = null;
        this._showPlaceholder = null;
        this._selectFirst = null;
        this._inputDom = null;
        this._listViewEl = null;
        this._mainListViewEl = null;
